...
procuraBlocoLivre:
    ...
    movq $0, %rdx # set %rdx para 0 pois ainda não achou um bloco

# Loop que procura blocos livres na heap
loop_ProcuraBlocoLivre:
    # Se ultrapassou o topo da heap
    cmpq %rcx, %rax
    jge fimLoopNaoAchou_ProcuraBlocoLivre
    # Se bloco está ocupado
    cmpq $1, (%rax)
    je loopOcupado_ProcuraBlocoLivre
    # Se não está ocupado e o bloco é menor
    addq $8, %rax
    cmpq %rdi, (%rax) # compara tamanho do bloco com o necessário p ser alocado
    jl loopEspacoMenor_ProcuraBlocoLivre
    # Senao, achou
    jmp LoopAchou_ProcuraBlocoLivre
